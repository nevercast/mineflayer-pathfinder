...
class Movements {
  constructor (bot, mcData) {
    ...
    this.gatewayBlocks = new Set()
    gateways.forEach(gateway => this.gatewayBlocks.add(mcData.blocksByName[gateway].id))
  ...
  getBlock (pos, dx, dy, dz) {
    ...
    b.height = pos.y + dy
    b.isGateway = this.gatewayBlocks.has(b.type)
    b.isPassable = (direction) => {
      debugger;
    };
  ...
  // Operate doors, gates and trapdoors
  getMoveGateways (node, dir, neighbors) {
    const block = this.getBlock(node, dir.x, 0, dir.z);
    if (!block.isGateway) return;
    if (block.isPassable(dir)) {
      neighbors.push(new Move(block.position.x, block.position.y, block.position.z, node.remainingBlocks, 1))
    }
  }
